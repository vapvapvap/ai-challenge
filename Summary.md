...
> **Ветка `L2`** (после merge `main` в `L2`): код соответствует `main` (все CLI-опции и `stop`), но конфиг по умолчанию — фиксированный `C:\Props\llm-cli.properties` (а не поиск `app.properties`), а `master_prompt.txt` (рядом с exe или в текущей папке) вставляется первым системным сообщением (`role=system`). Шаблон `app.properties` в `L2` содержит активные `temperature=0`, `top_p=0.01`, `thinking=disabled`.
