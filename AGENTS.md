...
- **CLI-опции (в `main` и производных `L1`/`L3`/`L4`/`L5`/`S1`)**: параметры можно указывать прямо в командной строке, они **перекрывают** файл настроек. Приоритет: **опция CLI > файл настроек > значение по умолчанию**.
