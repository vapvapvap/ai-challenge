...
- `L2` — от `L1` + влит `main` (доработки сохранены: системный `master_prompt.txt`, фиксированный путь конфига `C:\Props\llm-cli.properties`; из `main` получены все CLI-опции/`--stop`). Изначальная фича-версия — commit `7729aff`.
...
- В версии `L2`: конфиг по умолчанию — фиксированный `C:\Props\llm-cli.properties` (вне репозитория), а не поиск `app.properties`; `master_prompt.txt` ищется рядом с exe, затем в текущей папке, и вставляется первым как `role=system`. После влития `main` в `L2` CLI-опции (см. ниже) в ней присутствуют.
- **CLI-опции (в `main` и производных `L1`/`L2`/`L3`/`L4`/`S1`)**: параметры можно указывать прямо в командной строке, они **перекрывают** файл настроек. Приоритет: **опция CLI > файл настроек > значение по умолчанию**.
